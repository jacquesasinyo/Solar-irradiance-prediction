...
# Exclude target date from training data
...
knn_model = KNeighborsRegressor(
    ...
    algorithm='auto',    # Let algorithm choose the most appropriate search method
    leaf_size=30,        # Leaf size passed to BallTree or KDTree
    p=2,                 # Power parameter for the Minkowski metric (p=2 is Euclidean)
...
if len(day_data) == 0:
    ...
    predictions = knn_model.predict(X_pred_scaled)
    ...
    plt.plot(day_data['datetime'], predictions, 
             label='KNN Predicted', alpha=0.7, color='green')
    
    # Format x-axis to show hours
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
    plt.gca().xaxis.set_major_locator(mdates.HourLocator(interval=2))
    
    plt.title(f'Solar Irradiance Predictions (KNN) vs Actual Values {target_date}')
    ...
    # Save the plot
    plt.savefig('solar_predictions_knn.png')
    plt.close()
    ...
    print("\nTop 10 feature importance (KNN, based on permutation importance):")
    ...
    plt.ylabel('Feature')
    plt.title('Top 15 Feature Importance (KNN)')
    plt.tight_layout()
    ...
    day_data[['datetime', 'Global Horizontal [W/m^2]', 'Predicted_GHI']].to_csv(
        f'knn_predictions_{target_date}.csv', index=False,
        columns=['datetime', 'Global Horizontal [W/m^2]', 'Predicted_GHI'],
        header=['Datetime', 'Actual_GHI', 'Predicted_GHI']
    )
    print(f"Predictions saved to 'knn_predictions_{target_date}.csv'")
    ...
    if has_xgb or has_rf or has_ann:
        plt.figure(figsize=(15, 8))
        plt.plot(day_data['datetime'], day_data['Global Horizontal [W/m^2]'], 
        ...
        # Add KNN predictions
        plt.plot(day_data['datetime'], predictions, 
                label='KNN Predicted', alpha=0.7, color='green', linewidth=1.5)
        ...
        plt.savefig('model_comparison_plot_with_knn.png')
        plt.close()
        print("Comparison plot saved as 'model_comparison_plot_with_knn.png'")